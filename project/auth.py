...
def login_post():
    ...
    if not user:
        flash('ruh roh raggy, your details were rong...')
...
def signup_post():
    ...
    user = db.session.execute(db.select(User).filter_by(email=email)).first() # if this returns a user, then the email already exists in database
